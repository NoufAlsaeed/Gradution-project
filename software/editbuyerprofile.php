...
if(isset($_POST['update'])) // when click on Update button
{
  ...
  $phone = $_POST['phone'];
   ...
   if(is_numeric($phone) == false ){
    ...
    $edit = mysqli_query($db,"update buyer set Full_name='$name', email='$mail', phone_number='$phone' where id='$id'");
 
 }
  
    if($edit)
    {
      echo "<script type='text/javascript'>alert('تم تعديل المعلومات الشخصية بنجاح');location.href='buyerprofile.php'</script>"; 
      mysqli_close($db); // Close connection
        //header("location:buyerprofile.php"); // redirects to all records page
      //exit;
    }
    else
    {
...
<label for="hight" class = "text">:الاسم </label><br>
<input type="text" id="name" name="name" value="<?php 
$byuerId= $_SESSION['id']; $sqle1 = "SELECT * FROM buyer WHERE id = '$byuerId'"; 
$result1 = mysqli_query($db,$sqle1); $row1 = mysqli_fetch_assoc($result1);
...
$byuerId= $_SESSION['id']; $sqle1 = "SELECT * FROM buyer WHERE id = '$byuerId'"; 
$result1 = mysqli_query($db,$sqle1); $row1 = mysqli_fetch_assoc($result1);
echo"".$row1['email'].""; ?>"  maxlength='255' required >
</div>
<div class = "editeprofileContet">
