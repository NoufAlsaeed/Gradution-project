    ...
    <div class="profilePage">
        ...
        <div class="containerSVP">
            ...
            if (isset($_GET['search_key'])) {
                $key = $_GET['search_key'];
                include "db.php";
               $sql = mysqli_query($db, "SELECT * FROM `seller` WHERE `AccountState`='مصرح' and `ShopName` like '%$key%' ");
                $sql2 = mysqli_query($db, "SELECT * FROM `product` WHERE `productName` like '%$key%' ");
                if (mysqli_num_rows($sql) > 0) {

                    //echo '<h3 style="width: 50%; font-size: 20px;height:50px; padding: 10px; text-align:center; margin: 20px auto; background: darkcyan; color: #fff;">نتائج البحث</h3>';
                    while ($d = mysqli_fetch_array($sql)){
                        $shop_id = $d['id'];
                        $seller_name = $d['Full_name'];
                        $shop_name = $d['ShopName'];
                        $shop_image = $d['profilepic'];
                        echo '
                        <a href="shopbyseller.php?idd='.$shop_id.'" style="border: 1px solid #bbb;">
                            <h5 class="mainbag" style="width: 100%; height: 30px;padding: 5px; color: #000;">متجر</h5>
                            <div class="column">
                                <img class="pic" src="data:image/jpeg;base64,'.($shop_image).'" alt="seller"/>
                                <div class="buttonProfile btn-success" >
                                    <h5 class="mainbag" dir="rtl" >'.$shop_name.' <br/> '.$seller_name.' </h5>
                                </div>
                            </div>
                        </a>
                        ';
                ...
                elseif (mysqli_num_rows($sql2) > 0) {
                    while ($d2 = mysqli_fetch_array($sql2)){
                        $product_id = $d2['productID'];
                        $product_name = $d2['productName'];
                        $product_image = $d2['productPic'];
                        $productType = $d2['productType'];
                        echo '
                        <a href="productpage.php?idd='.$product_id.'" style="border: 1px solid #bbb;">
                            <h5 class="mainbag" style="width: 100%; height: 30px;padding: 5px;">'.$productType.'</h5>
                            <div class="column">
                                <img class="pic" src="data:image/jpeg;base64,'.($product_image).'" alt="seller"/>
                                <div class="buttonProfile btn-success">
                                    <h5 dir="rtl" >'.$product_name.' </h5>
                                </div>
                            </div>
                        </a>
                        ';
                    ...
                    echo '<h1 class="mainColor">لا توجد اى نتائج بحث عن: '.$_GET['search_key'].'</h1>';
                }
            }
            ?>
        </div>
