...
<main> 
	<?php  echo " 	<br><img  style='
 display: flex;
  text-align: right;
margin-left: 90%; ' height='35px' width='35px'  onclick='goBack()' src='images/arrowright.svg'>   <br> ";
?>
<br>
<h2 class="mb-5 pr-3 mt-3 text-center mainColor pp" style="font-size: 170%"> عرض المنتجات </h2>
...
$result1 = mysqli_query($db,$sqle1); 
$row1 = mysqli_fetch_assoc($result1);
echo"<h1 class='mb-5 pr-3 mt-3 text-center mainColor pp' style='font-size: 140%'>".$row1['ShopName']." </h1>";
...
<?php
$shopId=$_GET['idd'];
$sqle = "SELECT * FROM product WHERE sellerID = '$shopId' AND productType!='ادوات زراعية ' AND requestedToDeleteProduct='N' "; 
$check = [];
$index=0;
$isHere;
...
if (mysqli_num_rows($result) > 0){	 
          ...
          while ($row = mysqli_fetch_assoc($result)){
					  ...
					  if($row['sellerSizeID']==$value) {
					  $isHere=false;}
					  }
if($isHere==true){
			  $check[$index++] = $row['sellerSizeID'] ;

echo "<div class='product_wrapper'>";
echo "<div class='conOfPrcolumn'><a href='productpage.php?idd=".$row['productID']."'><img src='data:image/png;base64,".( $row["productPic"] )."' /></a> </div>";

    echo "
    <input type='hidden' name='productID' value=".$row["productID"]." />
	<input type='hidden' name='sellerID' value=".$row["sellerID"]." />
    <div class='name'>".$row["productName"]."</div>
    <div class='price'>السعر: ".$row["productPrice"]." ر.س</div>

    </div>";

}}
} else if (mysqli_num_rows($result) == 0) { 
echo '<br> <br> <p style="font-size: 150%; color:lightgrey"> لاتوجد نباتات لدى البائع في الوقت الحالي  </p> <br> <br> ';
}
?>
</div>
...
<div class = "profilePage">
<h2 class="typeOfP">معدات</h2>
<hr class="solid">
 <div class = "containerSVP">

<?php
$shopId=$_GET['idd'];
$sqle2 = "SELECT * FROM product WHERE sellerID='$shopId' AND productType='ادوات زراعية '  AND requestedToDeleteProduct='N' "; 
$check = [];
$index=0;
$isHere;
...
echo "<div class='product_wrapper'>";
echo "<div class='conOfPrcolumn'><a href='productpage.php?idd=".$row2['productID']."'> <img src='data:image/png;base64,".( $row2['productPic'] )."'/></a> </div>";
    ...
    <div class='price'>السعر: ".$row2["productPrice"]." ر.س</div>
    </div>";
 }} } else {
	
	echo '<br> <br> <p style="font-size: 150%; color:lightgrey"> لاتوجد ادوات زراعية لدى البائع في الوقت الحالي  </p> <br> <br> ';
	}
mysqli_close($db);
?>
</div>
